...
export default function SphereScene({
  ...
  useEffect(() => {
    ...
    // add 3D text "UPLOAD A RESUME!"
    const fontLoader = new FontLoader();
    fontLoader.load(
      // using a common Three.js example font path; replace if you have a local font
      'https://threejs.org/examples/fonts/helvetiker_regular.typeface.json',
      (font) => {
        const textGeometry = new TextGeometry('UPLOAD A RESUME!', {
          font: font,
          size: 0.3, // adjust size as needed
          depth: 0.05, // correct parameter for thickness
          curveSegments: 12,
          bevelEnabled: false,
        });
        ...
        // position text relative to the sphere (e.g., above it)
        textMesh.position.set(0, 1.5, 0); // adjust x, y, z for desired position around sphere
        
        sphere.add(textMesh); // add text as a child of the sphere to rotate together
      },
      undefined, // onProgress callback (optional)
      (error) => {
        console.error('Error loading font:', error);
        // fallback: add text using a canvas texture if font loading fails or as an alternative
        ...
        if (context) {
          ...
          context.font = "bold 30px Arial"; // slightly smaller font for this fallback
          context.textAlign = "center";
          context.fillText("UPLOAD A RESUME!", canvas.width / 2, canvas.height / 2 + 10);

          const texture = new THREE.CanvasTexture(canvas);
          const textPlane = new THREE.Mesh(
            new THREE.PlaneGeometry(2.5, 0.5), // adjusted plane size
          ...
          textPlane.position.y = 1.5; // position above the sphere
          sphere.add(textPlane); // add as child of sphere
        }
      }
    );
    ...
    const animate = () => {
      ...
      if (defaultObjectsRef.current && defaultObjectsRef.current.visible) {
        // sphere.rotation.y += 0.01; // keep this commented out
        // sphere.rotation.x += 0.005; // keep this commented out
      }
